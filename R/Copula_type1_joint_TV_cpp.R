...
type1<-function(idx,sim_num,iseed2,b1,b2,b3,b4){
        output<-list()
        for(iseed in 1:((sim_num)/100)){
                if (iseed>=13){iseed=iseed+1}
                data_example<-simulate.data(idx,iseed*iseed2,iseed*iseed2,1,b1,b2,b3,b4)
...
result<-type1(idx,sim_num,n.sim,0,1,0,1)
setwd("/Users/mengluliang/Dropbox/biostatisitcs_PSU/2021summerintern/BMS/03results")
saveRDS(result, file = paste0("res_joint_",idx,"_tvcop.RDS")) 
res_joint <- readRDS("res_joint_1_tvcop.RDS")
