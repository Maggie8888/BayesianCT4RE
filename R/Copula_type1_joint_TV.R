...
simulate.data<-function(idx,nsim,n.sim,kk){
        ...
        x1[,kk]=rbinom(n_patient,1,sd1[kk])###a contiunous covariate########
        x2[,kk]=rnorm(n_patient,0,sd2[kk]) ###a binary covariate############
        b01=rnorm(k,1,sd1[kk])
        b02=rnorm(k,1,sd1[kk])
        b1=rnorm(k,0.5,sd1[kk])
        b2=rnorm(k,1,sd1[kk])
        b3=rnorm(k,0.5,sd1[kk])
        b4=rnorm(k,1,sd1[kk])
        sigma_epsilon<-0.01
...
#################################################################################
data1<-simulate.data(1,1,1,1)
gamma.start=gamma=matrix(0.001,nrow=length(unique(data1$id)), ncol=1000)
type1<-function(sim_num,iseed2){
        output<-list()
        for(iseed in 1:((sim_num)/100)){
                if (iseed>=13){iseed=iseed+1}
                data_example<-simulate.data(1,iseed*iseed2,iseed*iseed2,1)
                output[[iseed]]<-para_est_TVJHCM(1,1,0,0,0,0,0.1,1,0.1,0.1,gamma.start,5000,100,20,iseed,
...
########################################################################
#install.packages("MASS",lib="/storage/home/m/mjl6416/R/x86_64-redhat-linux-gnu-library/3.6/",repos = "http://cran.us.r-project.org")
#install.packages("prodlim",lib="/storage/home/m/mjl6416/R/x86_64-redhat-linux-gnu-library/3.6/",repos = "http://cran.us.r-project.org")
#install.packages("survC1",lib="/storage/home/m/mjl6416/R/x86_64-redhat-linux-gnu-library/3.6/",repos = "http://cran.us.r-project.org")
...
#result[[n.sim]]<-type1(sim_num,n.sim)
#idx<-1
#setwd("/storage/work/m/mjl6416/BayesianCT/02results")
#saveRDS(result,paste0(idx,"_type1_TV.rds"))
